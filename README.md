...
Aplikacion mobile-first për bashkëpunimin mes mësimdhënësve dhe prindërve në shkollat e Kosovës. Qëllimi është ndjekja e vazhdueshme e nxënësit, me vëmendje të veçantë ndaj fëmijëve me nevoja të veçanta.
...
- Humori dhe njoftimi ditor i dërguar nga prindi.
...
- Supabase Auth, Database dhe Row Level Security
...
`OPENAI_API_KEY` dhe `SUPABASE_SERVICE_ROLE_KEY` duhet të ruhen vetëm në server ose në Supabase Secrets—kurrë në JavaScript-in e shfletuesit.
...
Skema, politikat RLS dhe të dhënat testuese gjenden në dosjen `supabase/`.
...
Projekti është prototip funksional dhe nuk është ende gati për përdorim me të dhëna reale shkollore. Para pilotimit duhen përfunduar testet e autorizimeve RLS, rrjedha e administratorit, ruajtja e PIA-s dhe rishikimi i privatësisë së të dhënave të fëmijëve.
...
- U përmirësuan ikonat, navigimi mobile, kontrasti dhe qasja me tastierë.
